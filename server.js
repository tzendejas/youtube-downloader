...
// === COOKIE CLONER ===
// Fixes "Read-only file system" error by copying cookies to a writable folder
...
async function downloadVideo(url, quality = 'best') {
    try {
        ...
        // We removed .%(ext)s and hardcoded .mp4 because we are forcing conversion
        ...
        // 1. We removed '[ext=mp4]' from quality selectors so it can grab WebM
        // 2. We added '--merge-output-format mp4' so it converts WebM to MP4 at the end
        ...
        if (quality === 'best') {
            // "Best video + best audio, OR just best single file"
        ...
        // Look for the specific timestamp file we just created
