// 🎬 GRABANYVIDEO - PRODUCTION VERSION (WITH SERVER COOKIES)
...
const COOKIES_PATH = '/etc/secrets/cookies.txt'; // Render Secret File Path
...
async function downloadVideo(url, quality = 'best') {
    try {
        ...
        // If the dummy account cookies exist, USE THEM. This is the #1 fix for IP blocks.
        if (fs.existsSync(COOKIES_PATH)) {
            console.log('🍪 using Server Cookies (Dummy Account) to authorize request...');
            command += ` --cookies "${COOKIES_PATH}"`;
        } else {
            console.warn('⚠️ No cookies found. Attempting unauthenticated download (High Risk of Block)');
            // Fallback: Try a generic client if no cookies (low success rate on Render)
...
app.get('/', (req, res) => {
    ...
    <body>
        ...
        <div class="info">Server Mode: Authenticated Production (Dummy Account)</div>
