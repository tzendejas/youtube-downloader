// 🎬 GRABANYVIDEO - TV MODE (Bypass "Sign In" & Network Fix)
...
async function downloadVideo(url, quality = 'best') {
    try {
        ...
        // === THE FIX: TV MODE ===
        // We removed --force-ipv6 (fixes network error)
        // We act like a Smart TV (bypasses "Sign in" prompt)
        ...
        command += ' --user-agent "Mozilla/5.0 (SMART-TV; LINUX; Tizen 5.0) AppleWebKit/537.3 (KHTML, like Gecko) SamsungBrowser/2.2 Chrome/63.0.3239.84 TV Safari/537.3"';
...
app.post('/api/download', async (req, res) => {
    try {
        ...
        if (msg.includes("Sign in")) {
            res.status(429).json({ success: false, error: "YouTube IP Blocked. Render IPs are flagged." });
        } else if (msg.includes("Network is unreachable")) {
            res.status(500).json({ success: false, error: "Network Configuration Error (IPv6 not supported)" });
        } else {
...
app.get('/', (req, res) => {
    ...
    <body>
        ...
        <div class="info">Server Mode: TV Spoofing (IPv4)</div>
