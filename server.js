// 🎬 GRABANYVIDEO - SORTING MODE (CRASH PROOF)
...
// IMPORTANT: Point to the FOLDER so yt-dlp finds both ffmpeg and ffprobe
...
async function downloadVideo(url, quality = 'best') {
    try {
        ...
        // === BUILD COMMAND ===
        // 1. Point ffmpeg-location to the BIN DIRECTORY (required for finding ffprobe)
        // 2. Use --merge-output-format mp4 to force standard container
        let command = `"${YTDLP_PATH}" "${url}" -o "${outputTemplate}" --ffmpeg-location "${BIN_DIR}" --merge-output-format mp4 --no-warnings --no-check-certificate --geo-bypass`;
        ...
        // === YOUTUBE FIX: BETTER QUALITY SELECTION ===
        // Use format selector with fallback instead of just sorting
        if (quality === 'best') {
            // Default behavior: Best video + Best audio
        } else if (quality === '4k') {
            // Try 4K first, if not available, fallback to best
            command += ' -f "bestvideo[height<=2160][vcodec^=avc1]+bestaudio/best[height<=2160]/best"';
        } else if (quality === '2k') {
            command += ' -f "bestvideo[height<=1440][vcodec^=avc1]+bestaudio/best[height<=1440]/best"';
        } else if (quality === '1080') {
            command += ' -f "bestvideo[height<=1080][vcodec^=avc1]+bestaudio/best[height<=1080]/best"';
        } else if (quality === '720') {
            command += ' -f "bestvideo[height<=720][vcodec^=avc1]+bestaudio/best[height<=720]/best"';
        } else if (quality === '480') {
            command += ' -f "bestvideo[height<=480][vcodec^=avc1]+bestaudio/best[height<=480]/best"';
        }
        ...
        if (downloadedFile) {
            ...
            throw new Error('File not created (YouTube might have blocked the IP or stream is live)');
        }
...
app.get('/', (req, res) => {
    ...
    <body>
        ...
        <div class="info">Server Mode: Robust Sorting (No Crashes)</div>
