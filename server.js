// 🎬 GRABANYVIDEO - FINAL PRODUCTION FIX (UNIVERSAL FORMAT SUPPORT)
...
async function downloadVideo(url, quality = 'best') {
    try {
        ...
        // Force .mp4 extension because we will convert it
        ...
        // === BUILD COMMAND ===
        // 1. --ffmpeg-location points to the FOLDER (fixes merge errors)
        // 2. --merge-output-format mp4 (Converts WebM to MP4 at the end)
        ...
        // === QUALITY SELECTION (THE FIX) ===
        // notice we REMOVED "[ext=mp4]" from the inputs.
        // This allows yt-dlp to download WebM source and convert it later.
        if (quality === 'best') {
            command += ' -f "bv*+ba/b"';
        } else if (quality === '4k') {
        ...
        if (errStr.includes("format is not available")) throw new Error("Could not find video stream.");
...
app.get('/', (req, res) => {
    ...
    <body>
        ...
        <div class="info">Server Mode: Universal Format Support</div>
