// 🎬 GRABANYVIDEO - SORTING MODE (THE FINAL FIX)
...
async function downloadVideo(url, quality = 'best') {
    try {
        ...
        // === BUILD COMMAND ===
        // FIX 1: Point ffmpeg-location to the BIN DIRECTORY (required for finding ffprobe)
        // FIX 2: Use --merge-output-format mp4 to force standard container
        ...
        // === QUALITY SELECTION (THE NEW FIX: SORTING) ===
        // Instead of -f (Format Selection) which crashes if not found,
        // We use -S (Sorting) which gracefully falls back to the next best option.
        if (quality === 'best') {
