// 🎬 GRABANYVIDEO - PRODUCTION VERSION (WITH WRITABLE COOKIES FIX)
...
// === COOKIE CLONER (THE FIX) ===
// Render secrets are read-only. We must copy them to a writable file.
...
function setupCookies() {
    try {
        if (fs.existsSync(SECRET_COOKIES)) {
            ...
            // Copy the file to our local folder where we have permission to write
            fs.copyFileSync(SECRET_COOKIES, WRITABLE_COOKIES);
            
            // Set permissions so yt-dlp can modify it (Read/Write for owner)
...
async function downloadVideo(url, quality = 'best') {
    try {
        ...
        // 2. Base Command
        let command = `"${YTDLP_PATH}" "${url}" -o "${outputTemplate}" --ffmpeg-location "${FFMPEG_PATH}" --no-warnings --no-check-certificate --geo-bypass`;

        // 3. Authentication Strategy
        if (hasCookies) {
            console.log('🍪 Using Authenticated Cookies (Writable Copy)...');
            command += ` --cookies "${WRITABLE_COOKIES}"`;
        } else {
            console.warn('⚠️ No cookies found. Using fallback Android mode.');
            command += ' --extractor-args "youtube:player_client=android"'; 
        }

        // 4. Quality Selection (Updated to be more flexible)
        // We added a fallback "/best" to the format string to prevent "Requested format not available" errors
        if (quality === 'best') {
            command += ' -f "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best"';
        } else if (quality === '1080') {
            command += ' -f "bestvideo[height<=1080][ext=mp4]+bestaudio[ext=m4a]/best[height<=1080][ext=mp4]/best"';
        } else if (quality === '720') {
            command += ' -f "bestvideo[height<=720][ext=mp4]+bestaudio[ext=m4a]/best[height<=720][ext=mp4]/best"';
        } else {
            command += ' -f "best[ext=mp4]/best"';
        }

        // 5. Standard User Agent
...
app.post('/api/download', async (req, res) => {
    try {
        ...
        if (msg.includes("Sign in") || msg.includes("429")) {
            res.status(429).json({ success: false, error: "Server authentication failed. Please update the cookies.txt secret file." });
        } else {
...
app.get('/', (req, res) => {
    ...
    <body>
        ...
        <div class="info">Server Mode: Authenticated Production (Writable Cookies)</div>
